...
export function resolveProvider(): ProviderSelection | null {
  const openaiKey = ENV("OPENAI_API_KEY");
  if (openaiKey) {
    const isOpenRouter = openaiKey.startsWith("sk-or-");
    return {
      provider: "openai",
      modelId: ENV("TUTOR_MODEL") ?? (isOpenRouter ? "openrouter/free" : "gpt-4o-mini"),
      baseUrl: (
        ENV("OPENAI_BASE_URL") ??
  ...
  const ollama = ENV("OLLAMA_HOST");
  if (ollama) {
    const host = ollama.replace(/\/+$/, "");
    return {
      provider: "ollama",
      modelId: ENV("TUTOR_MODEL") ?? "llama3.2",
